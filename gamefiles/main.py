...
The game is a simplistic stylised game reflecting something like space invaders however it is
an arena style combat game. There are multiples menus that the user will naviagte through. The 
...
#Character and ability variables
...
ability_start_time, ability_cooldown_started, ability_passed_time, is_active, can_use = [0, 0], [False, False], [0, 0], [False, False], [True, True]

#Enemy and enemy ability variables
...
explosion_x, explosion_y, explosion_radius, explosion_color = -100, -100, 64, [255, 255, 255]
...
#Powerup variables
powerup_type, powerup_x, powerup_y, powerup_length = [0, 1, 2], [-100, -100, -100], [-100, -100, -100], 64
...
#Menu variables
window, player_count, multiplayer, has_saved = 0, 0, False, False
...
#Draws the text that will show total player score
def draw_player_score(player_score):
    ...
    screen.blit(big_font.render("Score: " + str(sum(player_score)), True, BLACK), (500, 10))

#Draws the powerup cooldown bars
...
def draw_screen():
    ...
    elif multiplayer and main.is_frozen[1]:
...
def move_char(char):
    ...
    #Checks if user is pressing keys that moves characters, and moves the character in the desired direction
...
#Prevents player from moving if they are frozen
def frozen(char):
...
def use_ability(char):
    ...
    #Checks if the player can and is using an ability
...
def run_ability(char):
    ...
    if main.char_ability[char] == 1 and main.is_active[char] == True:
        ...
        #Makes the sword appear for 0.75 seconds
        ...
        if main.sword_passed_time[char] >= 750:
...
#Starts a cooldown and returns the amount of time that has passed since it started
...
def enemy_shoot_icicle(): 
    ...
    #Starts cooldown for when enemy can shoot again
...
def enemy_shoot_spike_ball(): 
    ...
    #Starts a cooldown between when enemy can shoot again
...
#Run the enemy invisible function
def enemy_invisible():
    import main
...
#Changes lime enemy size and speed when hit
...
#Run function for pirate to shoot cannon ball
def pirate_shoot_shot():
    ...
    #Starts cooldown for when enemy can shoot again
        ...
        if main.pirate_shot_passed_time >= 3000:
            main.pirate_shot_passed_time = 0
            main.pirate_shot_cooldown_started = False
            main.pirate_shot_can_shoot = True 

#Finds closest character and returns the x and y of the character as a list
...
def spawn_enemy(enemy_type):
    import main
    #Checks if the enemy is dead and it is their time to spawn
    if main.is_alive[enemy_type] == False and (main.gametime >= main.time_to_spawn[enemy_type]):

        #Resets boss enemy healths when they spawn
        if enemy_type == 7:
        ...
        #Makes invisible enemy visible
        if enemy_type == 6:
            main.invisible_value = 255

        #Picks 1 of 2 textures for pink enemy
        if enemy_type == 9:
        ...
        while True:
            ...
            if (multiplayer == False and chooseside != main.char_side[0]) or (multiplayer and chooseside != main.char_side[0] and chooseside != main.char_side[1]):
                if chooseside == 0:
...
def boss_collision_sorting(char, enemy_type, b_enemy_type):
    ...
    if main.b_enemy_health[b_enemy_type] == 1 and main.b_enemy_hit[char][b_enemy_type] == False:
        ...
        if b_enemy_type == 1:
            main.enemy_x[enemy_type], main.enemy_y[enemy_type] = 100000, 100000
            main.enemy_points[8] += 5
            main.gold_max_health += 1
    elif main.b_enemy_hit[char][b_enemy_type] == False:
        #Slightly changes position of lime enemy when hit to allign with change in size
        if b_enemy_type == 2:
...
#Run function to detect collision between player and spike ball
...
def enemy_ability_collision(char, enemy_type, t):
    ...
    #Starts a cooldown if the arrow hits an enemy
...
def powerup_player_collision(char, powerup_type):
    import main
    for powerup_xpos in range(int(main.powerup_x[powerup_type]), int(main.powerup_x[powerup_type] + main.powerup_length)):
        if main.char_x[char] + main.char_width >= powerup_xpos >= main.char_x[char]:
            for powerup_ypos in range(int(main.powerup_y[powerup_type]), int(main.powerup_y[powerup_type] + main.powerup_length)):
...
#Run function to reset variables when using a button to take you back to main menu
def reset_menu():
    import main
    import playMenu

    main.char_x, main.char_y, main.ability_x, main.ability_y, main.staff_x, main.staff_y = [size[0] / 2, size[0] / 2 + 100], [size[1] / 2, size[1] / 2], [[-100, -100], [-100, -100], [-100, -100]], [[-100, -100], [-100, -100], [-100, -100]], [-100, -100], [-100, -100]
    main.enemy_x, main.enemy_y, main.is_alive, main.enemy_points[8] = [-100, -100, -100, -100, -100, -100, -100, -100, -100, -100, -100, -100], [-100, -100, -100, -100, -100, -100, -100, -100, -100, -100, -100, -100], [False, False, False, False, False, False, False, False, False, False, False, False], 75
    ...
    main.char_health, main.char_face = [3, 3], [0, 0]
...
while rungame:
    ...
    #Gets the start time when game starts
    if window != 4 and window != 5:
        gamestart = pygame.time.get_ticks()

    #Changes amount of frames/sec depending on which window
    if window != 7:
    ...
    #Main game
        ...
        #Updates ability positions
        for i in range(0, 3):
            if char_ability[0] == i:
                use_ability(char[0])
                run_ability(char[0])
            if multiplayer and char_ability[1] == i:
                use_ability(char[1])
                run_ability(char[1])
        ...
        #Updates icicle position
        enemy_shoot_icicle()

        #Updates explosion position
        enemy_shoot_explosion()

        #Updates enemy spike ball position
        enemy_shoot_spike_ball()

        #Sets the invisible value for the invisible enemy
        enemy_invisible()

        #Changes size of lime enemy
        enemy_change_size()

        #Updates pirate shot position
        pirate_shoot_shot()
        ...
        if (multiplayer == False and char_health[0] <= 0) or (multiplayer and char_health[0] <= 0 and char_health[1] <= 0):
