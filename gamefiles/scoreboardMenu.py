...
def scoreboard_menu():
    import main
    from main import pygame, background, screen, LIGHT_GR, DARK_GR, BLACK, RED, GOLD, SILVER, BRONZE, size, menuselectsound, reset_menu, big_font, med_font, sml_font
    pygame.init()
    ...
    pygame.draw.rect(screen, BLACK, (970, 540, 210, 135), 5, 8)
    if 975 <= mouse[0] <= 1155 and 545 <= mouse[1] <= 670 and pressed[0] == True:
        menuselectsound.play()
        reset_menu()
        main.window = 0
        pygame.time.delay(100)
    elif 975 <= mouse[0] <= 1155 and 545 <= mouse[1] <= 670:
        pygame.draw.rect(screen, DARK_GR, (975, 545, 200, 125))
        screen.blit(text2, (1010, 595))
    else:
        pygame.draw.rect(screen, LIGHT_GR, (975, 545, 200, 125))
        screen.blit(text1, (1010, 595))